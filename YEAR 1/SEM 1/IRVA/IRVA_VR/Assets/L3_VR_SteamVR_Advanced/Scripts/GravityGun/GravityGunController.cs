// Fix for Meta XR build issues. SteamVR should anyway only be used on standalone builds.
#if UNITY_STANDALONE 
...
namespace L3_VR_SteamVR_Advanced.Scripts.GravityGun
{
    ...
    {
        [SerializeField] 
        [BoxGroup("Game object components")]
        [Required]
        private Interactable gunInteractable;
        ...
        private void OnEnable()
        {
            ...
            SteamVR_Actions._default.ClickTrigger.onChange += OnClickTrigger;
        }
        ...
        private void OnClickTrigger(SteamVR_Action_Boolean fromAction, SteamVR_Input_Sources fromSource, bool newState)
        {
            if (_selectedObject == null) 
                return;

            if(_selectedObject.IsSnapped && newState)
            {
                ThrowObject();
            }
        }

        private void OnTouchTrigger(SteamVR_Action_Boolean fromAction, SteamVR_Input_Sources fromSource, bool newState)
        {
            if (_selectedObject == null) 
                return;
