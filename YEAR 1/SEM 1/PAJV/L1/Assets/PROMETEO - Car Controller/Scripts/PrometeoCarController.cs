...
public class PrometeoCarController : MonoBehaviour
{
    ...
    [Space(10)]
    public InputType inputType; // The type of input that the user wants to use: keyboard or controller.
    ...
    */
    ...
    DrivingInputAction drivingInputActions;
        private float _throttleInput;
        private float _steeringInput;
        private float _brakeInput;
        private float _driftInput;

    public float speedNotifyInterval = 0.1f;
    private float speedNotifyTimer = 0f;
    private float lastNotifiedSpeed = -1f;

    private List<TickInput> _inputTicks = new List<TickInput>();
    ...
    */
    ...
    private void Awake() 
    {
      drivingInputActions = new DrivingInputAction();
      drivingInputActions.asset.bindingMask = inputType == InputType.KeyboardType ? InputBinding.MaskByGroup("Keyboard") : InputBinding.MaskByGroup("Controller");
    }

    private void OnEnable()
    {
        drivingInputActions.Enable();
    }

    private void OnDisable()
    {
        drivingInputActions.Disable();
    }
    ...
    void Update()
    {
        _throttleInput = drivingInputActions.Driving.Throttle.ReadValue<float>();
        _steeringInput = drivingInputActions.Driving.Steering.ReadValue<float>();
        _brakeInput = drivingInputActions.Driving.Brake.ReadValue<float>();
        _driftInput = drivingInputActions.Driving.Drift.ReadValue<float>();

        var color = inputType == InputType.KeyboardType ? "blue" : "green";
        //Debug.Log("<color=" + color + ">Input Type: " + inputType.ToString() + "Throttle: " + _throttleInput + " Steering: " + _steeringInput + " Brake: " + _brakeInput + " Drift: " + _driftInput + "</color>");
        ...
        speedNotifyTimer += Time.deltaTime;
        if (speedNotifyTimer >= speedNotifyInterval)
        {
            float absSpeed = Mathf.Abs(carSpeed);

            if (Mathf.Abs(absSpeed - lastNotifiedSpeed) > 0.05f)
            {
                OnSpeedChanged?.Invoke(absSpeed);
                lastNotifiedSpeed = absSpeed;
            }
            speedNotifyTimer = 0f;
        }
        ...
        */
        ...
        else
        {
            if (_isReplaying == false)
            {
                ...
                if ((_throttleInput == 0f && _brakeInput == 0f) && _driftInput == 0f && !deceleratingCar)
                {
                    InvokeRepeating("DecelerateCar", 0f, 0.1f);
                    deceleratingCar = true;
                }
                if (_steeringInput == 0f && steeringAxis != 0f)
                {
                    ResetSteeringAngle();
                }
    ...
    {
        if (!_isReplaying && isRecording)
        {
            var newTick = new TickInput
            {
    ...
    public void ApplyTickInput(TickInput tickInput)
    {
        _throttleInput = tickInput.throttle;
        _steeringInput = tickInput.steering;
        _brakeInput = tickInput.brake;
        _driftInput = tickInput.drift;
    ...
    public void SaveStartState()
    {
        ...
        {
            _startVelocity = carRigidbody.linearVelocity;
            _startAngularVelocity = carRigidbody.angularVelocity;
        }
        else
        {
    ...
    {
        if (_inputTicks == null || _inputTicks.Count == 0)
        {
            Debug.LogWarning($"{name} StartReplay: no recorded ticks to replay.");
            return;
        }

        if (_isReplaying)
        {
            Debug.LogWarning($"{name} already replaying.");
            return;
        }
    ...
    private IEnumerator ReplayCoroutine()
    {
        ...
        try { drivingInputActions.Disable(); } catch { }
        ...
        Debug.Log($"{name} Starting replay ({_inputTicks.Count} ticks).");
        for (int i = 0; i < _inputTicks.Count; i++)
        {
            var tick = _inputTicks[i];
            ApplyTickInput(tick);
            if (carRigidbody != null) carRigidbody.WakeUp();
            Debug.Log($"{name} Replay tick {i + 1}/{_inputTicks.Count}: Throttle={tick.throttle}, Steering={tick.steering}, Brake={tick.brake}, Drift={tick.drift}");

            yield return new WaitForFixedUpdate();
        }

        try { drivingInputActions.Enable(); } catch { }
        _isReplaying = false;

        Debug.Log($"{name} Replay finished ({_inputTicks.Count} ticks).");
    }
}
